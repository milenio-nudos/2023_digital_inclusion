...
Cronbach<-c(c2["alpha"],c3["alpha"],c3_1["alpha"],c3_2["alpha"],c3_3["alpha"],c3_4["alpha"],
  c4_a["alpha"],c4_b["alpha"],c4["alpha"],d2["alpha"],NA,NA,NA,NA,NA,NA,NA)

ci<-c(c2["ci"],c3["ci"],c3_1["ci"],c3_2["ci"],c3_3["ci"],c3_4["ci"],
            c4_a["ci"],c4_b["ci"],c4["ci"],d2["ci"],NA,NA,NA,NA,NA,NA,NA)

```


```{r,results='asis'}
raw_2023 %>% dplyr::select(starts_with(c("i_","extreme","c5"))) %>% 
...
model_data <- raw_2023 %>% dplyr::select(starts_with(c("i_","extreme","c5")),
                                         gender=a1,age=a2,ses=nse)%>%
              mutate(gender=ifelse(gender==2,1,0))%>%
              mutate_all(as.numeric)

corr_matrix<-cor(model_data%>%dplyr::select(-c(extreme_politization,gender,age,ses))%>%
                   drop_na())

testRes = cor.mtest(model_data%>%dplyr::select(-c(extreme_politization,gender,age,ses))%>%
