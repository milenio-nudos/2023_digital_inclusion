...
Además de lo anterior, la base de datos cuenta con escalas estimadas a partir de algunos de los ítems recién presentados. Estas escalas permiten al usuario trabajar con los principales constructos que busca medir el estudio, sin tener que procesar los datos por su cuenta.

-   La primera columna de la @tbl-index muestra el código de la columna que aparece en la base de datos. La segunda el nombre del constructo que indica la escala. 
...
**Se sugiere trabajar con estos índices para reducir la complejidad de los análisis en cuestión**.
...
names(index_table) <- gsub("^.{1,6}", "", names(index_table))
  ...
  tab_footnote(
    ...
    tab_footnote(
    footnote = "Construído a partir de variables módulo G",
